...
class ApiService {
  ...
  // Métodos de contenido con optimizaciones
  async getContent(params?: {
    ...
    const endpoint = `/api/content${queryParams.toString() ? `?${queryParams.toString()}` : ''}`;
    
    // Aplicar debounce solo para búsquedas
    if (params?.search) {
      const searchKey = `getContent_${params.search}_${params.category || ''}_${params.contentType || ''}`;
      return this.debounceSearch(searchKey, () => 
        this.request<PaginatedResponse<Content>>(endpoint, {}, true, true, 2 * 60 * 1000) // Cache por 2 min para búsquedas
      );
    }
    
    // Cache por 5 minutos para requests sin búsqueda
    return this.request<PaginatedResponse<Content>>(endpoint, {}, true, true, 5 * 60 * 1000);
  }

  async getContentById(id: string): Promise<ApiResponse<{ content: Content }>> {
    return this.request<{ content: Content }>(`/api/content/${id}`);
  }
  ...
  async getSubscriptions(params?: { page?: number; limit?: number; status?: string; creatorId?: string }): Promise<PaginatedResponse<Subscription>> {
    ...
    return this.request<PaginatedResponse<Subscription>>(endpoint);
  }
  ...
  async getNotifications(params?: { page?: number; limit?: number; search?: string }): Promise<PaginatedResponse<Notification>> {
    ...
    // Aplicar debounce solo para búsquedas
    if (params?.search) {
      const searchKey = `getNotifications_${params.search}`;
      return this.debounceSearch(searchKey, () => 
        this.request<PaginatedResponse<Notification>>(endpoint, {}, true, true, 1 * 60 * 1000) // Cache por 1 min para notificaciones
      );
    }
    
    // Cache por 1 minuto para notificaciones sin búsqueda
    return this.request<PaginatedResponse<Notification>>(endpoint, {}, true, true, 1 * 60 * 1000);
  }
