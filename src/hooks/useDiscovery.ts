...
export const useRecommendations = (limit: number = 10) => {
  ...
  const fetchRecommendations = useCallback(async (refresh: boolean = false) => {
    try {
      ...
      const response = await apiService.request('/api/recommendations', {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${apiService.getToken()}`,
          'X-Limit': limit.toString(),
          'X-Refresh': refresh.toString()
        }
      });
  ...
  const trackInteraction = useCallback(async (contentId: string, action: 'view' | 'like' | 'share' | 'comment') => {
    try {
      await apiService.request('/api/recommendations/interaction', {
        method: 'POST',
...
export const useTrending = (category?: string, limit: number = 20) => {
  ...
  const fetchTrending = useCallback(async () => {
    try {
      ...
      const response = await apiService.request('/api/discover/trending', {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${apiService.getToken()}`,
          'X-Category': category || '',
...
export const useDiscovery = (filters: DiscoveryFilters) => {
  ...
  const fetchDiscovery = useCallback(async (reset: boolean = false) => {
    try {
      ...
      const response = await apiService.request(`/api/discover?${params.toString()}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${apiService.getToken()}`
        }
...
export const useUserBehavior = () => {
  ...
  const fetchBehavior = async () => {
    try {
      ...
      const response = await apiService.request('/api/analytics/behavior', {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${apiService.getToken()}`
        }
  ...
  const trackSearchQuery = async (query: string) => {
    try {
      await apiService.request('/api/analytics/search', {
        method: 'POST',
  ...
  const trackSessionEnd = async () => {
    ...
    if (startTime) {
      ...
      try {
        await apiService.request('/api/analytics/session', {
          method: 'POST',
